...
type Model struct {
	...
	// csiSwallow is set when a terminal report (cursor position, DECRQM
	// mode report, ...) arrived split across reads: the decoder hands us
	// the CSI prefix as a uv.UnknownEvent and the parameter/terminator
	// bytes as individual key presses. While set, those trailing bytes
	// are dropped instead of leaking into the focused text field. See
	// handleSwallow.
	csiSwallow bool
...
// ErrPathRequired is returned by New when Config.Path is empty.
var ErrPathRequired = errors.New("ui: Config.Path is required")
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case uv.UnknownEvent:
		// A terminal report split across reads: the decoder could not
		// assemble it, so its CSI prefix surfaces here while the rest
		// follows as stray key presses. Arm the swallow so that tail does
		// not corrupt a focused input field.
		if isIncompleteCSI(string(msg)) {
			m.csiSwallow = true
		}
