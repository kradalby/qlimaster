...
// Export format selectors accepted by exportTo.
const (
	formatCSV  = "csv"
	formatXLSX = "xlsx"
	formatBoth = "both"
...
// format is one of formatCSV, formatXLSX or formatBoth. Errors are surfaced
// as inline status messages; the overlay is dismissed on success.
func (m Model) exportTo(format string) (tea.Model, tea.Cmd) {
	...
	if format == formatXLSX || format == formatBoth {
