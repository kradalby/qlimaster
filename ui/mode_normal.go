...
func (m Model) handleKey(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	// Drain the trailing bytes of a split terminal report before they can
	// reach any input field (see Model.csiSwallow).
	if m.csiSwallow {
		m2, swallowed := m.handleSwallow(msg.Code)
		if swallowed {
			return m2, nil
		}
...
func (m Model) handleExportKey(k string, km KeyMap) (tea.Model, tea.Cmd) {
	...
	case matches(km.ExportCSV, k):
		return m.exportTo(formatCSV)
	case matches(km.ExportXLSX, k):
		return m.exportTo(formatXLSX)
	case matches(km.ExportBoth, k):
		return m.exportTo(formatBoth)
	}
